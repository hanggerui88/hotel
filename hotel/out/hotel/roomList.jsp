...
<body>
    <div class="x-nav">
      ...
      <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right" href="roomList" title="刷新">
        <i class="layui-icon" style="line-height:30px">⥁</i></a>
    </div>
    <div class="x-body">
      <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" method="post" action="room_search">

            <input class="layui-input" placeholder="房间号码" name="rmNumber" id="rmNumber" /> >
      ...
      <xblock>
        ...
        <button class="layui-btn" onclick="x_admin_show('添加房型','./addRoom.jsp')"><i class="layui-icon"></i>添加</button>
      ...
      <table class="layui-table">
        ...
        <c:forEach items="${cs}" var="c">
          <tr>
            <td>
              <div name='cb' class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id=${c.rmNumber}><i class="layui-icon">&#xe605;</i></div>
            </td>
            <td name='rmNumber'>${c.rmNumber}</td>
            <td>${c.name}</td>
            <td>${c.image}</td>
            <td>${c.rmtype}</td>
              <td>${c.pf}</td>
              ...
              <td>${c.notes}</td>
            <td class="td-manage">
              <a title="查看"  onclick="x_admin_show('编辑','room_edit?rmNumber=${c.rmNumber}')" href="javascript:;">
                <i class="layui-icon">&#xe63c;</i>
              </a>
              <a deleteLink="true" title="删除" href="room_delete?rmNumber=${c.rmNumber}"  >
                <i class="layui-icon">&#xe640;</i>
              </a>
            </td>
          </tr>
    ...
    <script>
     ...
     $(".layui-form-checked").not('.header').parents('tr').remove();
  	 	window.location.href="room_delete_group?rmNumbers="+data;
  	}
  	}else{
